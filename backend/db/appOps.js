  ...
  try {
    if (columns.length === 0 || values.length === 0) {
      ...
      let query = await poolS
        ...
        .input("pageSize", sql.Int, pageSize)
        .query(
          `SELECT j.*, e.companyName FROM JobOpenings j JOIN Employer e ON j.company_id = e.id ORDER BY j.job_id OFFSET @offset ROWS FETCH NEXT @pageSize ROWS ONLY`
        );
      ...
      let query = await poolS
        ...
        .query(
          `SELECT j.*, e.companyName ` +
            queryString +
            ` ORDER BY j.job_id OFFSET @offset ROWS FETCH NEXT @pageSize ROWS ONLY`
        );
