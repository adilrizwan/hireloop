...
exports.getPosts = async (id, offset, pageSize) => {
  try {
    // let pool = await mssql.connect(config);
    let poolS = await pool;
    let query1 = await poolS
      ...
      .input("id", sql.Int, id)
      .query(
        "SELECT COUNT(job_id) AS TOTAL from JobOpenings where company_id = @id"
      );
    let query = await poolS
      .request()
      .input("id", sql.Int, id)
      ...
      .query(
        `SELECT * from JobOpenings where company_id = @id ORDER BY job_id OFFSET @offset ROWS FETCH NEXT @pageSize ROWS ONLY`
      );
    const fetched = await paginate.resultCount(
      offset,
      query.recordsets[0].length,
      query1.recordsets[0][0]["TOTAL"]
    );
    return {
      Results:
        "Showing " +
...
exports.updateApplicationStatus = async (jobID, empID, body) => {
  try {
    ...
    const result =
      await request.query(`IF EXISTS (SELECT 1 FROM JobOpenings WHERE (job_id = @jobID and company_id = @empID))
                                          BEGIN   
...
exports.getJobAndApplicantDetails = async (jobID, empID, offset, pageSize) => {
  try {
    // let pool = await mssql.connect(config);
    let poolS = await pool;
    let query = await poolS
      ...
      .input("pageSize", sql.Int, pageSize)
      .query(
        `exec GetJobAndApplicantDetails @jobID, @empID, @offset, @pageSize`
      );
    const fetched = await paginate.resultCount(
      offset,
      query.recordsets[2].length,
      query.recordsets[1][0]["NUMBER_OF_APPLICANTS"]
    );
    if (query.recordset.length === 0) {
      return 0;
    } else {
      return {
        JobPost: query.recordsets[0],
        TotalApplicants: query.recordsets[1][0]["NUMBER_OF_APPLICANTS"],
        Results:
          "Showing " +
          fetched +
          " of " +
